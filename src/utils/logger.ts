...
export function printHeader(): void {
  for (const line of buildBanner("Sessions", undefined, {
    Scope: "all projects",
  })) {
    console.log(line);
  }
...
export function printSummary(
  ...
  console.log(
    lastSub(
      `${color.bold(String(totalProjects))} projects, ${color.bold(String(totalSessions))} sessions, ${color.bold(hours + "h")} total`,
    ),
  );
  console.log();
}
...
function printMessage(msg: NormalizedMessage): void {
  ...
  const roleColor = msg.role === "user" ? color.bold : secondary;
  ...
  for (const tool of msg.toolUses) {
    ...
    console.log(
      tertiary(`  ${tool.name}${inputPreview ? ": " + inputPreview : ""}`),
    );
  }
...
export function printNoRulesFound(): void {
  printWarning("No CLAUDE.md or .claude/rules/ found in this directory.");
  console.log();
  console.log(secondary("  ContextLinter needs rules files to analyze."));
  console.log();
  console.log(
    secondary(
      "    /init  — run in Claude Code in this directory to set up CLAUDE.md",
    ),
  );
  console.log();
  console.log(secondary('  Then run "contextlinter init" again.'));
  console.log();
}
...
export function printAnalysisDone(
  insightCount: number,
  durationSec: number,
  isLast = true,
): void {
  if (insightCount > 0) {
    console.log(
      success(
        `${insightCount} insight${insightCount === 1 ? "" : "s"} found ${secondary(`(${durationSec.toFixed(1)}s)`)}`,
        isLast,
      ),
    );
  } else {
    if (isLast) {
      console.log(
        lastSub(secondary(`No insights found (${durationSec.toFixed(1)}s)`)),
      );
    } else {
      console.log(
        substep(secondary(`No insights found (${durationSec.toFixed(1)}s)`)),
      );
    }
  }
}
...
export function printSkippedSummary(count: number): void {
  if (count > 0) {
    console.log(
      substep(
        secondary(
          `Skipped ${count} session${count === 1 ? "" : "s"} (too short)`,
        ),
      ),
    );
  }
...
export function printSessionsDebugList(
  ...
  for (const s of sessions) {
    ...
    const size = formatFileSize(s.fileSize);
    console.log(
      treeCont(tertiary(`Session ${shortId} | ${date}${source} | ${size}`)),
    );
  }
}
...
export function printInsightResults(
  ...
  for (let ci = 0; ci < categories.length; ci++) {
    ...
    printCategoryBlock(
      category,
      insights,
      ci === categories.length - 1 && !hasCross,
    );
  }
  ...
  const totalAnalysisTime = results.reduce(
    (sum, r) => sum + r.stats.analysisTimeMs,
    0,
  );
  const sessionsWithInsights = results.filter(
  ...
  console.log(
    substep(
      `${color.bold(String(allInsights.length))} insights from ${sessionsWithInsights}/${results.length} sessions` +
        (crossPatterns.length > 0
          ? `, ${color.bold(String(crossPatterns.length))} cross-session patterns`
          : ""),
    ),
  );
  console.log(
    lastSub(
      secondary(
        `Analysis time: ${(totalAnalysisTime / 1000).toFixed(1)}s total`,
      ),
    ),
  );
  console.log();
...
function printInsight(insight: Insight, isLast: boolean): void {
  ...
  console.log(
    connector(
      `${pLabel}  ${color.bold(String(confPct) + "%")}  ${insight.title}`,
    ),
  );
...
function printCrossPattern(
  ...
  console.log(
    connector(
      `${color.bold(String(confPct) + "%")}  ${pattern.title} ${secondary(`(${sessionCount} sessions)`)}`,
    ),
  );
...
export function printAnalysisSummaryLine(
  ...
  const skipMsg =
    alreadyAnalyzed > 0
      ? secondary(` (${alreadyAnalyzed} already analyzed, skipped)`)
      : "";
  console.log(substep(`Project: ${shortPath(projectPath)}`));
  console.log(
    substep(`Sessions to analyze: ${color.bold(String(toAnalyze))}${skipMsg}`),
  );
}
...
export function printDryRun(
  ...
  console.log(
    substep(
      secondary(
        `[dry-run] Would analyze ${shortId} (${userMessages} user msgs) ${icon.dash} ${reason}`,
      ),
    ),
  );
}
...
export function printNothingToAnalyze(): void {
  console.log(
    lastSub(
      secondary("Nothing new to analyze. Run with --force to re-analyze."),
    ),
  );
  console.log();
}
...
export function printRulesOverview(snapshot: RulesSnapshot): void {
  ...
  // Files
  const nameWidth = Math.max(...snapshot.files.map(f => f.relativePath.length));
  const scopeWidth = Math.max(...snapshot.files.map(f => f.scope.length));
  const ruleCountWidth = Math.max(...snapshot.files.map(f => {
    return `${f.rules.length} rule${f.rules.length === 1 ? '' : 's'}`.length;
  }));

  console.log(step(`Rules files: ${snapshot.files.length}`));
  for (let i = 0; i < snapshot.files.length; i++) {
    const file = snapshot.files[i];
    const name = file.relativePath.padEnd(nameWidth);
    const scope = file.scope.padEnd(scopeWidth);
    const ruleCount = `${file.rules.length} rule${file.rules.length === 1 ? '' : 's'}`.padEnd(ruleCountWidth);
  ...
  const largeFiles = snapshot.files.filter(
    (f) => f.content.split("\n").length > 500,
  );
  if (largeFiles.length > 0) {
    ...
    for (const file of largeFiles) {
      ...
      console.log(
        warn(
          `${file.relativePath} is very large (${lineCount} lines). Consider splitting into .claude/rules/.`,
        ),
      );
    }
  }
  ...
  statLines.push(
    `By scope: ${stats.byScope.global} global, ${stats.byScope.project} project, ${stats.byScope.project_local} local, ${stats.byScope.subdirectory} subdir`,
  );
  statLines.push(
    `By format: ${stats.byFormat.bullet_point} bullet, ${stats.byFormat.paragraph} paragraph, ${stats.byFormat.command} command, ${stats.byFormat.emphatic} emphatic`,
  );
  ...
  const emphaticCount = snapshot.allRules.filter(
    (r) => r.emphasis === "important",
  ).length;
  if (emphaticCount > 0) {
    statLines.push(
      `Emphasis: ${emphaticCount} rules with IMPORTANT/MUST/NEVER`,
    );
  }
...
function printRulesFileDetail(file: RulesFile): void {
  console.log(
    color.bold(
      `${filePath(file.relativePath)} (${file.scope}, ${file.rules.length} rule${file.rules.length === 1 ? "" : "s"})`,
    ),
  );
  console.log();
  ...
  for (const rule of file.rules) {
    if (rule.section !== currentSection) {
      ...
      if (currentSection) {
        const sectionRules = file.rules.filter(
          (r) => r.section === currentSection,
        );
        console.log(
          color.bold(`  ## ${currentSection} (${sectionRules.length} rules)`),
        );
      }
    }
    ...
    const textPreview =
      rule.text.length > 100 ? rule.text.slice(0, 97) + "..." : rule.text;
...
export function printRulesFileLargeWarning(
  path: string,
  lineCount: number,
): void {
  console.log(
    warn(
      `${path} is very large (${lineCount} lines). Consider splitting into .claude/rules/.`,
    ),
  );
}
...
export function printSuggestionLoadingSummary(
  ...
  console.log(
    substep(
      `Insights: ${color.bold(String(insightCount))} (${sessionInsightCount} from sessions, ${crossPatternCount} cross-session patterns)`,
    ),
  );
  console.log(
    substep(
      `Rules: ${color.bold(String(ruleCount))} rules across ${fileCount} file${fileCount === 1 ? "" : "s"}`,
    ),
  );
  if (filteredOut > 0) {
    console.log(
      substep(
        `Filtered: ${insightCount} insights (${filteredOut} below confidence threshold)`,
      ),
    );
  }
}
...
export function printSuggestionResults(
  ...
  if (stats.byType.consolidate > 0)
    typeCounts.push(`${stats.byType.consolidate} consolidate`);

  console.log(
    step(
      `Summary: ${stats.total} suggestion${stats.total === 1 ? "" : "s"} (${typeCounts.join(", ")})`,
    ),
  );
  if (stats.insightsSkipped > 0) {
    console.log(
      substep(
        secondary(
          `${stats.insightsSkipped} insights skipped (already covered)`,
        ),
      ),
    );
  }
  console.log(
    lastSub(
      secondary(
        `Current rules: ${stats.estimatedRulesAfter - stats.total} ${icon.arrow} estimated after: ${stats.estimatedRulesAfter}`,
      ),
    ),
  );
  console.log();
...
function printSuggestionFileGroup(
  file: string,
  suggestions: Suggestion[],
): void {
  console.log(
    color.bold(
      `${filePath(file)} (${suggestions.length} change${suggestions.length === 1 ? "" : "s"})`,
    ),
  );
  console.log();
...
function printSuggestionItem(s: Suggestion): void {
  ...
  console.log(
    `  ${pLabel}  ${color.bold(String(confPct) + "%")}  ${typeLabel}: ${s.title}`,
  );
...
export function printSuggestionBatchProgress(
  batchIdx: number,
  batchCount: number,
): void {
...
export function printSuggestionGenerated(
  count: number,
  skippedCount: number,
  durationSec: number,
): void {
  if (count > 0) {
    const skippedMsg =
      skippedCount > 0
        ? ` (${skippedCount} skipped ${icon.dash} already covered)`
        : "";
    console.log(
      success(`${count} suggestion${count === 1 ? "" : "s"}${skippedMsg}`),
    );
  } else {
    console.log(
      lastSub(
        secondary(`No suggestions generated (${durationSec.toFixed(1)}s)`),
      ),
    );
  }
  console.log();
